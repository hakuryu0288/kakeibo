...
import { Subscription, PlannedExpense, WishItem, BigExpense, CreditCard } from '@/lib/supabase'
...
export default function PlansPage() {
  const [tab, setTab] = useState<'sub' | 'planned' | 'wish' | 'big'>('sub')
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-1 bg-slate-100 rounded-xl p-1">
        {([['sub','サブスク'],['planned','確定出費'],['wish','欲しいもの'],['big','大型出費']] as const).map(([key, label]) => (
            ...
            <div className="space-y-3">
              ...
              {plannedExpenses.map((p) => (
                <div key={p.id} className={`bg-white rounded-xl p-3 shadow-sm flex justify-between items-center ${p.is_done ? 'opacity-50' : ''}`}>
                  <div>
                    <p className="text-sm font-medium line-through-if-done">{p.name}</p>
            ...
            <div className="space-y-3">
              ...
              {showBigForm ? (
                <form onSubmit={async (e) => { e.preventDefault(); await post('/api/big-expenses', { name: bigForm.name, amount: parseInt(bigForm.amount), planned_date: bigForm.planned_date || null, note: bigForm.note || null }); setBigForm({ name: '', amount: '', planned_date: '', note: '' }); setShowBigForm(false) }} className="bg-white rounded-xl p-4 shadow-sm space-y-3">
                  ...
                  <input type="text" placeholder="項目名（例：PC購入）" value={bigForm.name} onChange={(e) => setBigForm({ ...bigForm, name: e.target.value })} className="w-full border border-slate-200 rounded-lg p-2 text-sm" required />
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <input type="date" placeholder="予定日" value={bigForm.planned_date} onChange={(e) => setBigForm({ ...bigForm, planned_date: e.target.value })} className="border border-slate-200 rounded-lg p-2 text-sm" />
                  </div>
